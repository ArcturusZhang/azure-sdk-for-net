...
namespace Azure.Communication.CallingServer
{
    internal class PartitionedDownloader
    {
        ...
        {
            DiagnosticScope scope = _client._clientDiagnostics.CreateScope($"{nameof(PartitionedDownloader)}.{nameof(ConversationClient.DownloadToAsync)}");
            try
            {
                scope.Start();
        ...
        internal Response DownloadTo(
            ...
            CancellationToken cancellationToken)
        {
            DiagnosticScope scope = _client._clientDiagnostics.CreateScope($"{nameof(PartitionedDownloader)}.{nameof(ConversationClient.DownloadTo)}");
            try
            {
                scope.Start();
