...
namespace Azure.ResourceManager
{
    ...
    [ClientTestFixture]
    [NonParallelizable]
    public abstract class ComputeTestBase : ManagementRecordedTestBase<ComputeManagementTestEnvironment>
    {
        ...
        protected ComputeTestBase(bool isAsync) : base(isAsync, RecordedTestMode.Record)
        {
        }

        private Subscription _defaultSubscription;

        protected Subscription DefaultSubscription => _defaultSubscription ??= GetDefaultSubscription();

        private ResourceGroupContainer _resourceGroupContainer;

        protected ResourceGroupContainer ResourceGroupContainer => _resourceGroupContainer ??= GetResourceGroupContainer();

        private ResourceGroupContainer GetResourceGroupContainer(ArmClientOptions clientOptions = default)
        {
            return GetDefaultSubscription(clientOptions).GetResourceGroups();
        }

        private Subscription GetDefaultSubscription(ArmClientOptions clientOptions = default)
        {
            return GetArmClient(clientOptions).DefaultSubscription;
        }
